...
const clientCall = twilio(accountSid, apiKeySid, apiKeySecret);
const clientText = twilio(accountSid, process.env.TWILIO_AUTH_TOKEN);
...
app.post("/send-sms", async (req, res) => {
  ...
  try {
    const response = await clientText.messages.create({
      body: message,
...
app.get("/get-sms", async (req, res) => {
  try {
    const messages = await clientText.messages.list(); // Fetch the latest 10 messages
...
app.get("/get-single-sms/:sid", async (req, res) => {
  ...
  try {
    const message = await clientText.messages(sid).fetch(); // Fetch the specific message
...
wss.on("connection", (ws) => {
  ...
  ws.send(JSON.stringify({ type: "status", message: "Waiting for call..." }));
...
// app.post("/callHandler", (req, res) => {
//   const twiml = new twilio.twiml.VoiceResponse();

//   // Respond with a simple message when the call is received
//   twiml.say("Hello! Please wait while we connect your call.");
...
app.get("/fetch-emails", async (req, res) => {
  try {
    storedEmails = await fetchLatestEmails(); // Correctly store emails
